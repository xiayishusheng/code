...
int main(int argc, char *argv[])
{
    ...
    while (err != -1)
    {
        ...
        { //10111111 11111111 11101101 11101000
            char ipbuf[255];
            inet_ntop(AF_INET, &((struct sockaddr_in *)&(ifr.ifr_ifru.ifru_addr))->sin_addr, ipbuf, sizeof(ipbuf));
            printf("the %dst interface is:%s \t addr:%s\n", ifr.ifr_ifindex, ifr.ifr_name, ipbuf);
        }
    }
    //传入网络接口名称，获得标志
    bzero(&ifr, sizeof(struct ifreq));
    ...
    {
        printf(
                "SIOCGIFMAP,mem_start:%ld,mem_end:%ld,base_addr:%d,ifr_map:%d,dma:%d,port:%d\n",
